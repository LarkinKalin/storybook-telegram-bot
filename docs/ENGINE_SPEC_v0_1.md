...
история была ограниченной по длине (8/10/12 шагов),

выбор ребёнка накапливал последствия (через скрытые параметры),

история вела к понятному финалу (5 финалов),

при шуме/ерунде игра не ломалась, а мягко закрывалась.
...
content_delta_clamped=true (если пришло от кнопок/контента),

classifier_delta_clamped=true (если пришло от free-text).
...
продолжать историю так, будто выбран один из A/B/C (внутренне),

не вводить четвёртый путь и новые типы действий,

не усиливать последствия (никаких “навсегда потерял друга”),

держать “безопасный ход” по тону.
...
текст матчится regex:

только точки/многоточия/пробелы: ^[\s\.\…]+$

один “паразит” (без учёта регистра):
{ "не знаю","незнаю","хз","лол","ок","угу","ээ","а","мм","...","…" }

классификатор вернул tag noise (доп. сигнал)
...
доминанта и финал воспроизводимы по логам,

формулы и примеры вех совпадают,

определены enum’ы (neutral_reason, milestone_vote_reason),

noise детерминирован,

есть две матрицы: финалов и дельт,

есть fail-safe для JSON классификатора,

определено поведение при leaderCore=tie и коллизиях M6/M7.
