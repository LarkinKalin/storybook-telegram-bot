...
### Дополнение TG.8.1.02.B (asset path resolver)
- В рендере PDF добавлен `_resolve_asset_file_path(asset_row)` с fallback-кандидатами для кривых `storage_key`.
- Если файл изображения не найден, рендер **не падает**: пишется warning `book.pdf missing image file ...`, страница рендерится текстом.
- `_pick_style_reference(...)` теперь возвращает только `asset_id`, для которого реально найден файл на диске.
